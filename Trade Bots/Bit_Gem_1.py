...
while True:
	...
	btc_amt_2 = (float(min_amt_1)*float(gem_sell_price))
	btc_amt_3 = (float(min_amt_1)*float(bit_sell_price))
	btc_amt_4 = (float(min_amt_2)*float(bit_buy_price))
	...
	def gem_sell_order():
		...
		print "GEMINI: sold", min_amt_2, "ETH for", btc_amt_2, "BTC --- Rate, ETH/BTC =", gem_sell_price

	def bit_sell_order():
		bit_trading_client.sell_limit_order(min_amt_1, bit_sell_price)
		print "BITSTAMP: sold", min_amt_1, "ETH for", btc_amt_3, "BTC --- Rate, ETH/BTC =", bit_sell_price

	def bit_buy_order():
		bit_trading_client.buy_limit_order(min_amt_2, bit_buy_price)
		print "BITSTAMP: bought", min_amt_2, "ETH for", btc_amt_4, "BTC --- Rate, ETH/BTC =", bit_buy_price
	...
	if R1>0:
		...
		loop_time = arrow.now()
		print "Time Elapsed Since Start:", (loop_time - start_time)
